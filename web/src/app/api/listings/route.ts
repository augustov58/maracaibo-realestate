...
interface QueryParams {
  ...
  min_sqm?: number;
  max_sqm?: number;
  price_changed?: boolean;  // Filter for price drops/changes
  sort_by?: string;
  page?: number;
  limit?: number;
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filter for price changes (current price different from original)
    if (params.price_changed) {
      whereClause += ' AND original_price IS NOT NULL AND price_usd IS NOT NULL AND original_price != price_usd';
    }
